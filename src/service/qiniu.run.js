...
const QiniuUpload = function (ak, sk, bk, list, url) {
    console.log(`total ${list.length} files`.cyan);
    ...
    list.forEach((item, index) => {
        config.formUploader.putFile(config.uploadToken, item.key, item.path, config.putExtra, function (respErr, respBody, respInfo) {
            if (respErr) {
                console.log(``);
                console.log(colors.red(respErr));
                console.log(``);
            }
            if (respInfo.statusCode == 200) {
                console.log(colors.green(`===> ${url?url:'~'}/${respBody.key} done`));
            } else {
                console.log(``);
                console.log(colors.yellow(`${respInfo.statusCode}: ${respBody} done`));
                console.log(``);
            }
        })
    });
